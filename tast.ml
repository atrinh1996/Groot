...
and string_of_tyvar = function TVariable n -> "'" ^ string_of_int n
...
and string_of_constraint (t1, t2) =
  "(" ^ string_of_ttype t1 ^ ", " ^ string_of_ttype t2 ^ ")"
...
and string_of_tx = function
  ...
  | TypedVar id -> id
  | TypedIf (te1, te2, te3) ->
      "(if " ^ string_of_texpr te1 ^ " " ^ string_of_texpr te2 ^ " "
      ^ string_of_texpr te3 ^ ")"
  | TypedApply (f, args) ->
      "(" ^ string_of_texpr f ^ " ["
  ...
  | TypedLet (binds, body) ->
      ...
      in
      "(let ("
      ^ String.concat " " (List.map string_of_binding binds)
      ^ ") " ^ string_of_texpr body ^ ")"
  | TypedLambda (formals, body) ->
      let formalStringlist =
        List.map (fun (ty, x) -> string_of_ttype ty ^ " " ^ x) formals
      in
      "(lambda ("
      ^ String.concat ", " formalStringlist
      ^ ") " ^ string_of_texpr body ^ ")"
...
and string_of_ttree = function
  | TLeaf -> "leaf"
  | TBranch (v, sib, child) ->
      "(tree " ^ string_of_tvalue v ^ " " ^ string_of_ttree sib ^ " "
