...
let rec sexprToCexpr ((ty, e) : sexpr) = match e with 
  ...
  | SVar s                  -> (ty, CVar s)
  | SIf (s1, s2, s3)        -> (ty, CIf (sexprToCexpr s1, sexprToCexpr s2, sexprToCexpr s3))
  | SApply (fname, args)    -> (ty, CApply (fname, List.map sexprToCexpr args))
  | SLet (bs, body)         -> (ty, CLet (List.map (fun (x, e) -> (x, sexprToCexpr e)) bs, sexprToCexpr body))
...
let svalToCval (id, (ty, e)) = 
  ...
  (match e with 
    | SLambda (fformals, fbody) -> 
        ...
        in 
        let _ = StringMap.add (id ^ string_of_int (find id res.rho)) f_def res.functions
        in None 
